...
function parseArgs(): {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--weeks') {
      if (!args[i + 1]) {
        console.error('--weeks requires a value')
        process.exit(1)
      }
      ...
      if (!args[i + 1]) {
        console.error('--teacher requires a value')
        process.exit(1)
      }
      ...
      if (!args[i + 1]) {
        console.error('--shift requires a value')
        process.exit(1)
      }
      ...
      if (
        ...
        isNaN(new Date(dateArg + 'T12:00:00').getTime())
      ) {
        console.error(
          `--exclude must be a valid YYYY-MM-DD date, got: ${dateArg}`
        )
        process.exit(1)
      }
      ...
      console.error(
        'Usage: analyze-checkins [--weeks N] [--teacher NAME] [--shift MORNING|AFTERNOON] [--exclude YYYY-MM-DD]'
      )
      process.exit(1)
    }
  }
...
function getWeekendDatesInRange(weeksBack: number): string[] {
  ...
  // T12:00:00 (no offset) parses in the Node.js process timezone. Noon is chosen because
  // no UTC offset shifts noon across a date boundary, making getDay()/getDate() reliable.
...
async function getCheckins(
  ...
  return prisma.dugsiTeacherCheckIn.findMany({
    where: {
      ...
      // @db.Date in Postgres is date-only. new Date('YYYY-MM-DD') parses as UTC midnight
      // per ECMA spec, which Prisma maps correctly to the date value.
      date: {
...
function classifyCheckins(
  ...
  for (const c of checkins) {
    // @db.Date comes back as UTC midnight; formatting in UTC recovers the original stored date without timezone shift.
  ...
  for (const teacher of teachers) {
    ...
    for (const date of weekendDates) {
      for (const shift of shifts) {
        ...
        let recalcLate: boolean
        let minutesLate: number
        try {
          const result = evaluateCheckIn({
            clockInTimeUtc: checkin.clockInTime,
            shift,
          })
          recalcLate = result.isLate
          minutesLate = result.minutesLate
        } catch (err) {
          throw new Error(
            `evaluateCheckIn failed for teacher ${teacher.id} date ${date} shift ${shift}: ${err instanceof Error ? err.message : String(err)}`
          )
        }
...
function printReport(
  ...
  if (excluded.length > 0) {
    console.log(
      `Excluded dates: ${excludeDates.size} (${excluded.length} check-ins excluded)`
    )
  }
  console.log()
  console.log(
  ...
  console.log(
    `  Late:        ${String(allLate.length).padStart(4)} (${pct(allLate.length, totalExpected)})`
  )
  console.log(
  ...
  if (mismatches.length > 0) {
    ...
    console.log(sep)
    console.log('These check-ins have a stored isLate flag that disagrees with')
  ...
  if (missing.length > 0) {
    ...
    console.log()
    console.log(padRight('Teacher', 20) + padRight('Date', 14) + 'Shift')
  ...
  if (allLate.length > 0) {
    ...
    for (const b of brackets) {
      ...
      console.log(
        `  ${padRight(b.label + ':', 12)} ${count} (${pct(count, allLate.length)})`
      )
    }
  }
...
async function main() {
  console.log(
    `[READ-ONLY] Connecting to: ${process.env.DATABASE_URL?.replace(/\/\/.*@/, '//***@') ?? 'unknown'}`
  )

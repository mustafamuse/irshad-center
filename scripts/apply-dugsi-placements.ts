...
const PLACEMENTS: Placement[] = [
  // → 101/Morning (Mustafa Awil)
  {
    ...
    mode: 'move',
    fromClass: { name: '[A.M] Khalid/Abdirahim', shift: M },
  },
  {
    name: 'Khalid Abdirahman',
    target: { name: 'Mustafa Awil', shift: M },
    mode: 'place',
  },
  {
    name: 'Abdifatah Abdirahman',
    target: { name: 'Mustafa Awil', shift: M },
    mode: 'place',
  },
  {
  ...
  {
    name: 'Zainab Hajijama',
    target: { name: 'Ducale Matan', shift: M },
    mode: 'place',
  },
  {
    name: 'Sahraa Hajijama',
    target: { name: 'Ducale Matan', shift: M },
    mode: 'place',
  },
  {
  ...
  {
    name: 'Aisha Osman',
    target: { name: 'Suraya Mohamed', shift: A },
    mode: 'place',
  },
  {
    name: 'Samia Aden',
  ...
  // correction: move 101/Morning → 101/Afternoon
  {
    ...
    target: { name: 'Mustafa Awil', shift: A },
    mode: 'move',
    fromClass: { name: 'Mustafa Awil', shift: M },
  },
  // correction: "Adil" on Mohamed tab = Adil Shimoye; move 104/Morning → 103/Morning
  {
    ...
    target: { name: 'Mohamed Ali-Daar', shift: M },
    mode: 'move',
    fromClass: { name: 'Abdiwahab Haibah', shift: M },
  },
]
...
async function resolve(p: Placement): Promise<Resolved> {
  ...
  if (!tId)
    return {
      ...base,
      note: `target class ${p.target.name}/${p.target.shift} not found`,
    }
  base.targetClassId = tId

  const profiles = await prisma.programProfile.findMany({
    where: { program: Program.DUGSI_PROGRAM, person: { name: p.name } },
    select: {
      id: true,
  ...
  const already = byName.find(
    (x) =>
      x.dugsiClassEnrollment?.isActive && x.dugsiClassEnrollment.classId === tId
  )
  if (already) {
    return {
      ...base,
      profileId: already.id,
      currentClass: `${p.target.name}/${p.target.shift}`,
      action: 'skip-already-there',
      note: 'already enrolled in target',
    }
  }
  ...
  if (candidates.length !== 1) {
    return {
      ...base,
      note: `expected 1 ${p.mode} candidate, found ${candidates.length} (of ${profiles.length} by name)`,
    }
  }
  ...
  base.currentClass = enr?.isActive
    ? `${enr.class.name}/${enr.class.shift}`
    : null

  if (p.mode === 'move')
    return { ...base, action: 'move', note: `from ${base.currentClass}` }
...
async function main() {
  ...
  for (const r of resolved) {
    ...
    console.log(
      `[${tag}] ${r.placement.name} → ${r.placement.target.name}/${r.placement.target.shift}  ${r.note}`
    )
  }
  ...
  if (errors.length) {
    throw new Error(
      `${errors.length} placement(s) failed to resolve — aborting (no writes).`
    )
  }

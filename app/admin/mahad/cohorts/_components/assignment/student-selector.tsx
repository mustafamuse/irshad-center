...
export function StudentSelector({
  ...
  const isStudentSelected = useCallback(
    (id: string) => selectedStudentIds.has(id),
    [selectedStudentIds]
  )

  // Filter students based on mode and batch selection (memoized)
  const sourceStudents = useMemo(
    ...
    [mode, students, selectedBatch]
  )

  const destinationStudents = useMemo(
    () =>
      mode === 'assign'
  ...
  const toggleStudent = useCallback(
    ...
    [selectedStudentIds, selectStudent, deselectStudent]
  )

  const sourceTitle = useMemo(
    () =>
      mode === 'assign' ? 'Available Students' : 'Students in Source Cohort',
    [mode]
  )
  ...
  const destinationTitle = useMemo(
    () =>
      mode === 'assign'
        ? 'Students in Target Cohort'
        : 'Students in Destination Cohort',
    [mode]
  )

  // Memoized empty callback for non-selectable cards
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="flex h-[400px] flex-col">
        <div className="mb-4 space-y-2">
          <h3 className="text-sm font-medium">{destinationTitle}</h3>
          <Input
            placeholder="Search students..."

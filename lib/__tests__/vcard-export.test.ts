...
describe('generateVCardsContent', () => {
  it('should generate multiple vCards joined with CRLF', () => {
    const contacts = [
      { firstName: 'John', lastName: 'Doe', fullName: 'John Doe' },
      { firstName: 'Jane', lastName: 'Doe', fullName: 'Jane Doe' },
    ]

    const content = generateVCardsContent(contacts)

    expect(content).toContain('FN:John Doe')
    expect(content).toContain('FN:Jane Doe')
    expect(content.split('BEGIN:VCARD').length).toBe(3)
  })

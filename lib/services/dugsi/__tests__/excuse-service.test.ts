  ...
  return {
    excuseRequest: {
      ...
      findUniqueOrThrow: (...args: unknown[]) =>
        mockExcuseFindUniqueOrThrow(...args),
...
vi.mock('@/lib/db/queries/teacher-attendance', async (importOriginal) => {
  ...
  return {
    ...actual,
    getAttendanceRecordStatus: (...args: unknown[]) =>
      mockGetRecordStatus(...args),
    getExistingActiveExcuse: (...args: unknown[]) =>
      mockGetExistingActiveExcuse(...args),
  }
})
...
describe('submitExcuse', () => {
  ...
  it('throws EXCUSE_NOT_ELIGIBLE when teacherIds do not match', async () => {
    mockGetRecordStatus.mockResolvedValue({
      id: 'rec-1',
      teacherId: 't-other',
      status: 'ABSENT',
    })
  ...
  it('throws ALREADY_EXCUSED when a PENDING or APPROVED excuse exists', async () => {
    ...
    mockGetExistingActiveExcuse.mockResolvedValue({
      id: 'ex-1',
      status: 'PENDING',
    })
  ...
  it('throws ALREADY_EXCUSED on P2002 (two concurrent excuse submissions race past the pre-check)', async () => {
    // Both teachers pass getExistingActiveExcuse (no active excuse yet), but the
    // second insert hits the partial unique index on (attendanceRecordId WHERE PENDING/APPROVED).
    ...
    mockGetExistingActiveExcuse.mockResolvedValue(null)
    mockExcuseCreate.mockRejectedValue(
...
describe('approveExcuse', () => {
  ...
  it('throws ATTENDANCE_RECORD_NOT_FOUND when attendance record was deleted after excuse was found', async () => {
    ...
    mockFindUniqueRecord.mockResolvedValue(null)
  ...
  it('throws CONCURRENT_MODIFICATION when attendance record was modified concurrently', async () => {
    ...
    mockExcuseFindUniqueOrThrow.mockResolvedValue({
      id: 'ex-1',
      status: 'APPROVED',
    })

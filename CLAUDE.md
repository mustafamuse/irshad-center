...
Per Erik Schluntz's leaf-node restriction pattern. Claude may edit these only with explicit user direction; auto-modes should treat them as read-only. Enforced by `permissions.ask` rules in `.claude/settings.json` — edits to these paths always prompt, even in auto-accept mode.
...
- **Settlement fields record when money moved, not when we heard about it.** `Subscription.paidUntil` means "paid through" and has exactly one writer: the invoice webhook, gated on the invoice being settled. Creation derives it from `resolveInitialPaidUntil` (only `active` counts); every path that updates an existing row omits the field rather than writing it, including the `update:` branch of the `createSubscription` upsert — a caller that believes it is inserting can land there in a race and erase a genuinely paid period. The same rule governs `Subscription.amount`: a full withdrawal sets `cancel_at_period_end`, so Stripe still bills the final period and the amount must not be zeroed until cancellation actually lands. When changing any of this, grep every write site (`grep -rn 'paidUntil:' lib/`) — a gate added to one path does not propagate, and upsert conflict branches are easy to miss.

- **Stripe payload shapes must be read defensively.** The SDK pins `2025-08-27.basil`, which moved `invoice.subscription` to `invoice.parent.subscription_details.subscription` and `current_period_*` from the subscription onto its items. Webhook payloads render at whatever version each _endpoint_ is configured for, so both shapes arrive concurrently — a production sample showed roughly a 50/50 split. Read both, via `extractInvoiceSubscriptionId` and `extractPeriodDates`. Reading only one silently produces nulls for half of live traffic.
